 ...
 * @author Benoit
 */
public class CtrlPlaceQuiz implements WindowListener, ActionListener, KeyListener {

    private VuePlaceQuiz vue; // LA VUE
    ...
    private Timer timer;
    private CtrlPlaceQuiz capitalQuiz = this;
    ...
    private int ANSWER_WAIT = 3000;
    private int RIGHT_ANSWER_WAIT = 2000;
    ...
    public CtrlPlaceQuiz(VuePlaceQuiz vue, CtrlPrincipal ctrl, int mode, int difficulty) {
        this.vue = vue;
        ...
        try {
            ...
            Logger.getLogger(CtrlPlaceQuiz.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            ...
            rightClip.open(AudioSystem.getAudioInputStream(CtrlPlaceQuiz.class.getResource("/sounds/" + "right.wav")));
            wrongClip.open(AudioSystem.getAudioInputStream(CtrlPlaceQuiz.class.getResource("/sounds/" + "wrong.wav")));
            timeOutClip.open(AudioSystem.getAudioInputStream(CtrlPlaceQuiz.class.getResource("/sounds/" + "timeOut.wav")));
            clockClip.open(AudioSystem.getAudioInputStream(CtrlPlaceQuiz.class.getResource("/sounds/" + "clock.wav")));
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    AudioInputStream inputStream = AudioSystem.getAudioInputStream(CtrlPlaceQuiz.class.getResource("/sounds/" + url));
    ...
    private void stop() {
     ...
     /* vue.getContentPane().setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            ...
            if (userAlreadyPlayed) {
                if (personalBest > score) {
                    try {
                        ...
                        Logger.getLogger(CtrlPlaces.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    scoreAmeliore = true;
        ...
        vue.getjLabelClassement().setVisible(false);
    ...
    private void display(boolean isPlaying) {
        ...
        vue.getjLabelPays().setVisible(false);
    ...
    private void doRandom() {
        ...
        while (i < NB_ANSWERS) {
            ...
            if (countriesBis.get(index).getCapital() != null && notSame && !countryAlreadyAsked) {
                boolean flagLoaded = true;
                if (i == 0) {
                    country = countriesBis.get(index);
                    answerIndex = indexButtons.get(i);
                    vue.getjLabelPays().setText(country.getCountryName());
                    try {
                        ImageIcon imageIcon = new ImageIcon(getClass().getResource("/images/places/" + country.getIsoAlpha3()+ ".jpg")); // load the image to a imageIcon
    ...
    private void changeButtonAnswer(int indexButton) {
        switch (indexButton) {
            case 0:
                vue.getjButtonCapitale1().setText(countriesBis.get(index).getCountryName());
                ...
                break;
            case 1:
                vue.getjButtonCapitale2().setText(countriesBis.get(index).getCountryName());
                /* if (countriesBis.get(index).getCapital().length() < 16) {
                 Font f = new Font(vue.getjButtonCapitale1().getFont().getName(), Font.BOLD, 14);
                ...
                break;
            case 2:
                vue.getjButtonCapitale3().setText(countriesBis.get(index).getCountryName());
                /*if (countriesBis.get(index).getCapital().length() < 16) {
                 Font f = new Font(vue.getjButtonCapitale1().getFont().getName(), Font.BOLD, 14);
                ...
                break;
            case 3:
                vue.getjButtonCapitale4().setText(countriesBis.get(index).getCountryName());
                /* if (countriesBis.get(index).getCapital().length() < 16) {
                 Font f = new Font(vue.getjButtonCapitale1().getFont().getName(), Font.BOLD, 14);
    ...
    private void checkAnswer(int indexButton) {
        ...
        vue.getjLabelPays().setVisible(true);
        vue.getjLabelPays().setText(country.getPlace());
    ...
    public VuePlaceQuiz getVue() {
        return vue;
    }

    public void setVue(VuePlaceQuiz vue) {
        this.vue = vue;
    }
